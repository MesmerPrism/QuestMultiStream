...
for (const page of pages) {
    ...
    await fs.writeFile(page.outputPath, renderPage(page, pages), "utf8");
}
...
function renderPage(page, pages) {
    ...
    const navSections = groupedPages.map(([groupName, entries]) => {
        const items = entries.map(candidate => {
            ...
            return `<a class="nav-item${activeClass}" href="${relativeHref}"${active}><strong>${escapeHtml(candidate.navLabel)}</strong><span>${escapeHtml(candidate.summary)}</span></a>`;
        }).join("");
    ...
    const brandMarkHref = `${assetPrefix}/assets/quest-multi-stream-mark.png`;
...
<head>
  ...
  <title>${escapeHtml(page.title)} | Quest Multi Stream</title>
  ...
  <div class="site-shell">
    <header class="site-header">
      <a class="brand" href="${homeHref}">
        <img class="brand-mark" src="${brandMarkHref}" alt="Quest Multi Stream icon">
        <div class="brand-copy">
          <span class="brand-title">Quest Multi Stream</span>
          <span class="brand-subtitle">Quest casting operator surface for Windows</span>
        </div>
      </a>
    </header>
    ...
    <main class="page-layout">
      ...
      <article class="content-panel prose">
        <p class="eyebrow">${escapeHtml(page.navGroup)}</p>
